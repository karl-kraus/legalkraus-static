...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    xmlns:utils="http://example.com/utils" version="2.0" exclude-result-prefixes="#all">
                        ...
                        <section class="row py-3 {if (count(//tei:pb) = 0 and count(//tei:graphic) = 0) then 'invisible' else ()}">
                            <div class="col-12" id="reader" data-page="1">
                                <div class="d-flex justify-content-center align-items-center gap-3 mb-3">
                                    <div>
                                        <a href="{$teiSource}">
                                            <i class="fas fa-download fa-lg" title="show TEI source"/>
                                        </a>
                                    </div>
                                    <div class="ms-2">
                                        <span>
                                            <span>
                                                <button id="prevPage" onclick="Pager.selectPage('prev')" class="me-1 bg-white border-0 align-middle">
                                                    <i class="fas fa-arrow-left fa-lg" title="go to previous page"/>
                                                </button>
                                            </span>
                                            <xsl:text>Seite </xsl:text>
                                            <xsl:variable name="select_enabled">
                                                <xsl:value-of select="count(//tei:pb) > 1 or (not(//tei:pb) and count(//tei:graphic[@source = 'wienbibliothek']) > 1)" />
                                            </xsl:variable>
                                            <select id="page-selector" onchange="Pager.selectPage()" class="form-select form-select-sm w-auto ml-2 d-inline" aria-label="page selector">
                                                <xsl:if test="$select_enabled = false()">
                                                    <xsl:attribute name="disabled"/>
                                                </xsl:if>

                                                <xsl:for-each select="//tei:pb">
                                                    <xsl:apply-templates select="current()" mode="page-selector"/>
                                                    <xsl:variable name="page_nr" select="count(preceding::tei:pb) + 1"/>
                                                </xsl:for-each>
                                                <xsl:if test="count(//tei:pb) = 0">
                                                    <xsl:for-each select="//tei:graphic[@source = 'wienbibliothek']">
                                                        <option value="{current()/@url}">
                                                            <xsl:value-of select="count(current()/preceding::tei:graphic[@source = 'wienbibliothek']) + 1" />
                                                        </option>
                                                    </xsl:for-each>
                                                </xsl:if>
                                            </select>
                                            ...
                                            <button id="nextPage" onclick="Pager.selectPage('next')" class="ms-1 bg-white border-0 align-middle">
                                                <i class="fas fa-arrow-right fa-lg" title="go to next page"/>
                                            </button>
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div id="container-resize" class="row transcript active gy-2 {if (count(//tei:pb) = 0 and count(//tei:graphic) = 0) then 'd-none' else ()} mx-auto">
                                ...
                                <div id="text-resize" class="col-12 text yes-index">
                                                        ...
                                                        <div class="{if (count(preceding::tei:pb) = 0) then 'd-block' else 'd-none'} doc-page position-relative" id="page_{count(preceding::tei:pb) + 1}">
                                                            <xsl:choose>
                                                                <xsl:when test="$nextPb">
                                                                    <xsl:apply-templates select="utils:split(current(), $nextPb, //tei:TEI)" />
                                                                </xsl:when>
                                                                <xsl:otherwise>
                                                                    <xsl:apply-templates select="utils:split(current(), (//tei:TEI//element())[last()], //tei:TEI)" />
                                                                </xsl:otherwise>
                                                            </xsl:choose>
                                                        </div>
                                                    </xsl:for-each>
    ...
    <xsl:template match="tei:rdg">
        ...
        <span class="rdg {if (tei:note[@rend='leftMargin']) then 'marginLeft' else ('mRight')}">
            ...
            <a id="show-btn" href="#" data-bs-toggle="modal" data-bs-target="#witnessModal{replace(@wit,'#','')}" class="d-block witness-link text-secondary">Textzeuge</a>
        </span>
    </xsl:template>
</xsl:stylesheet>